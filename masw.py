...
#PARTE 2: INVERSIÓN
    ...
    def theoretical_dispersion_curve(self, c_test, h, alpha, beta, rho, n, lambda_curve0):
    ...
    def error(self, c_t, lambda_t):
        e = np.mean(np.sqrt((self.c_curve0 - c_t)**2) / self.c_curve0) #Error de la Velocidad de fase
        #e2 = np.mean(np.sqrt((self.lambda_curve0 - lambda_t)**2) / self.lambda_curve0) #Error de la Longitud de Onda
    ...
    #def inversion(self, c_test, h, alpha, beta, rho, n, up_low_boundaries):
        ...
        #trazar curvas de dispersión teóricas y experimentales
        #FigWidth = 8 #cm
        #FigHeight = 10 #cm
        #FigFontSize = 8 #pt
        #self.plot_theor_exp_dispersion_curves(c_t, lambda_t, up_low_boundaries, FigWidth, FigHeight, FigFontSize)
        
        #Calcule el desajuste [%] entre las curvas de dispersión teórica y experimental
        #e = self.misfit(c_t)
        #fig, ax = plt.subplots()
        #ax.set_title('Misfit: ϵ = ')

        #i = int(input('Inserte el número con la opción correspondiente: \n 1. Iterar \n 2. Guarde la curva de dispersión teórica (en un archivo de texto) y deje de iterar \n 0. Dejar de iterar sin guardar la curva de dispersión teórica (en un archivo de texto): '))

        #if (i == 1):
            #ii = int(input('1. Cambiar el grosor de las capas y la velocidad de las ondas de corte \n 2. Cambiar la velocidad de la onda de corte \n 3. Cambiar grosores de capa: '))
            #if (ii == 1):
                #print('Espesores de capa en la última iteración')
                #print('%6.2f', h)
                #print('\n')
                #h = input(['Ingrese un nuevo vector de espesor de capa (de longitud ', str(n),') ' ])
                
                #print('\n')
                #print('Velocidades de onda de corte en la última iteración')
                #print('%8.2f', beta)
                #print('\n')
                #beta = input(['Ingrese un nuevo vector de velocidad de onda de corte (de longitud ', str(n+1), ') ' ])
        ...
        #c_t, lambda_t, e = self.inversion(c_test,h,alpha,beta,rho,n,up_low_boundaries)

        #elif (i == 2):
        #A = np.array([c_t.T, lambda_t].T)
        #fileID = open('Results.txt', 'w')
        #print(fileID,'%16s %16s\r\n','c (theor.)', 'lambda (theor.)')
        #print(fileID,'%16.4f %16.4f\r\n', A)
        #fileID.close()
        
        #return c_t, lambda_t, e